...
- スマートフォン優先のReact SPAとして提供し、LINEログイン時は投稿をクラウド保存する。公開とプッシュ通知は対象外とする。
...
- IndexedDBは全データの端末保存とキャッシュを担う。LINEログイン中の投稿はD1、画像は非公開R2にも保存する。
- WorkerがOAuth callback、招待制登録、session、投稿APIの所有者認可を担う。地図タイル提供者へ投稿データは送信しない。
- 文字数、画像形式・枚数、空投稿をUIと保存前で検証する。

## 品質上の制約

- 画像をWeb Storageへ置かず、投稿単位でIndexedDBへ保存する。保存失敗時は既存データと入力状態を維持する。
- 未ログインのローカルデータはoriginとブラウザに依存する。ログイン時の投稿同期は実装済みだが、日記・設定を含む完全同期、バックアップ、PWA、施設検索は未実装。
