...
- 地図の写真ピンまたは通常ピンをタップすると、場所名のツールチップは表示せず、ピンを拡大し、ドロワー展開後の表示可能範囲の中央へ投稿地点を移動してから、ドロワーを開いて該当投稿まで移動する
...
地図上のピンはドラッグで位置を微調整でき、移動後も利用者が調整した地図の中心と表示倍率を維持します。Shouldとして、現在地付近へ地図を移動してからピンを指定できる操作を加えます。

新規投稿時に位置情報権限がすでに許可されている場合は、現在地を初期場所として地図へズーム16で表示します。未許可の場合は自動で権限を要求せず、地図上の現在地ボタンを選択したときに要求し、取得成功時は同じ倍率で表示します。
...
地図または地図の空状態を画面上部へ大きく配置し、地図上にはカードを重ねず、投稿ピンを操作しても場所名のツールチップを表示しない。その下のおもいで投稿領域の先頭に、曜日付きの「○年○月○日（○）の日記」と、その日に登場するぬいの円形アイコンおよび「とおでかけ」を、上余白9pxかつ小さめの文字によるコンパクトなヘッダーとして表示する。投稿と一日のまとめは角丸カードで表示する。日記保存後に投稿が変更された場合も、更新通知文は表示しない。
...
- 投稿写真をタップまたはkeyboardで選択すると、ヘッダー、下部メニュー、ドロワーより前面の全画面プレビューを開き、保存画像を本来の縦横比で画面内へ収めて表示する。
