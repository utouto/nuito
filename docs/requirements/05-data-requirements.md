...
| themeColor | CSS color/string | No | アイコンの囲み線の色。初期値は `transparent` |
...
- LINEログイン中の投稿・ぬい・日記・設定はD1にも保存し、画像Blobは所有者名前空間の非公開R2へ保存する。
- R2 object keyはサーバーで生成し、D1に保持した所有関係を確認してから画像を返す。
