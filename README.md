...
ログインせずに利用する場合、データは同じブラウザ・同じoriginのIndexedDBへ保存されます。LINEログイン中は投稿、ぬい、日記、設定をD1、投稿画像・サムネイル・ぬいアイコンを非公開R2にも保存し、IndexedDBを端末側キャッシュとして利用します。
...
- LINEログイン中は投稿・画像・ぬい・日記・設定をクラウドにも保存します。プッシュ通知は将来拡張です。
